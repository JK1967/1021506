#####Functions from Chadi & Krause, 2018. Adapted for the thesis by me. 
...
## RD Functions ####
#difference in three plots is the cut-off of the data. Emp and ent are more zoomed in than the other plots. 
jump.plot.emp <- function( data , force.var , yvar , seat.identifier , polynomial, ylabel ){
  data <- data[ , c( force.var , yvar  , seat.identifier )]
  data <- na.omit( data )
  library( ggplot2 )
  p <- ggplot( ) +
    geom_point( data = data 
                , aes_string( x = force.var, y = yvar , color = seat.identifier ) 
                , size = 1.5 ) +
    geom_smooth( data = subset( data , data[ , force.var] < 0 )
                 , aes_string ( x = force.var , y = yvar )
                 , method = 'loess'
                 , linetype = 1 , color = 'black' , size = 1 , alpha = 0.5 ) +
    geom_smooth( data = subset( data , data[ , force.var] >= 0 )
                 , aes_string ( x = force.var , y = yvar )
                 , method = 'loess'
                 , linetype = 1 , color = 'black' , size = 1 , alpha = 0.5 ) +
...
jump.plot.blur <- function( data , force.var , yvar , seat.identifier , polynomial, ylabel ){
  data <- data[ , c( force.var , yvar  , seat.identifier )]
  data <- na.omit( data )
  library( ggplot2 )
  p <- ggplot( ) +
...
jump.plot.ent <- function( data , force.var , yvar , seat.identifier , polynomial, ylabel ){
  data <- data[ , c( force.var , yvar  , seat.identifier )]
  data <- na.omit( data )
  library( ggplot2 )
  p <- ggplot( ) +
